...
Tool use in particular is interesting.  There are *more* tool use lines in the Chic version (Claude code omits many tool use lines), but they don't get in the way, and each is expandable for full context.  Additionally, Markdown renders properly.
...
Unfortunately, no one knows how they work.  We've all seen some article or Youtube video showing them in action, but in practice they're a foreign workflow.  That's ok; AI tooling is great at foreign workflows.
...
Git worktrees solve this by marrying a git branch with a fresh copy of the repository directory.  When you clean up the branch the directory is removed as well:
...
Now you can have `n` Claude agents working, one in each directory.  When you have a new idea, immediately create a new worktree and have Claude start on it:

```
/worktree my-idea
```

This creates a new worktree (directory + branch) and a new agent working in that directory.  The agent starts working concurrently while I go back to my previous task.  In practice I often have 2-10 of these going at once.

Now I focus on whatever ongoing thread interests me most; Claude pushes the others forward.  I no longer wait for Claude; it waits for me.

### Cleaning up

Concurrent development means conflicts.  My git flow:

1.  Finish work in a branch, commit
2.  Rebase on our upstream branch.
    - Resolve any conflicts from recently merged work
3.  Fast-forward merge branch into upstream
4.  Delete worktree

Claude handles rebases and conflicts fine (and asks when it can't).  The workflow above is baked into `/worktree finish`.
...
Cute demo, but in practice I use multiple agents with prompts like the following:

-  **Reviewing:** "Start an agent to review our changes critically"
-  **Research:** "Start an agent to explore this topic"
-  **Dogfooding:** When adding a profiler, we spawned an agent to use it and suggest improvements back to the agent building it
...
Other niceties: `/diff` mode, vim keybindings (`/vim`), shell commands (`!ls`, `!nvim README.md`) without leaving the app.
